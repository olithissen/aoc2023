...
public class Main implements Runnable {
    ...
    private static final Function<String, Integer> getCardId = string -> Integer.valueOf(string.replaceAll("Card", "").trim());
    private static final Function<String, List<Integer>> splitNumbers = string -> Arrays.stream(string.split(" ")).map(String::trim).filter(s -> !s.isEmpty()).map(Integer::valueOf).toList();

    private static final Function<String, Card> extractCards = line -> {
        var elements = line.split(":");
        var numbers = elements[1].split("\\|");

        return new Card(getCardId.apply(elements[0]), splitNumbers.apply(numbers[0]), splitNumbers.apply(numbers[1]), 0);
    };
    private static final ToIntFunction<Card> countIntersections = card -> {
        return (int)card.ownNumbers.stream().filter(card.winningNumbers::contains).count();
    };
    ...
    private static final Function<List<String>, Object> solution2 = input -> {
        var cardInventory =  input.stream()
                .map(extractCards)
                .map(card -> new Card(card.id, card.winningNumbers, card.ownNumbers, countIntersections.applyAsInt(card)))
                .collect(Collectors.toMap(Card::id, Function.identity()));

        List<Card> process = cardInventory.values().stream().toList();

        do {
            process = process.stream().flatMap(card -> {
                var newOriginal = new Card(card.id, card.winningNumbers, card.ownNumbers, 0);
                var copies = IntStream.rangeClosed(card.id + 1, card.id + card.wins).mapToObj(cardInventory::get).toList();
