...
public class Main implements Runnable {
    ...
    private static final Function<List<String>, List<Position>> extractSymbolPositions =
            getStringPositions("[^\\d\\.]+");
    private static final Function<List<String>, List<Position>> extractGearPositions = getStringPositions("\\*");
    private static final Function<Position, Position> extendRange = position ->
            new Position(position.x1 - 1, position.y1 - 1, position.x2 + 1, position.y1 + 1, position.value);
    /**
    ...
    private static final Function<List<String>, Object> solution1 = input -> {
        List<Position> numbers =
                extractNumberPositions.apply(input).stream().map(extendRange).toList();
        List<Position> symbols =
                extractSymbolPositions.apply(input).stream().map(extendRange).toList();
    ...
    private static final Function<List<String>, Object> solution2 = input -> {
        List<Position> numbers =
                extractNumberPositions.apply(input).stream().map(extendRange).toList();
        List<Position> gears =
                extractGearPositions.apply(input).stream().map(extendRange).toList();

        return gears.stream()
                .mapToInt(number -> {
                    var hits = numbers.stream().filter(number::intersects).toList();
                    return hits.size() == 2
                            ? Integer.parseInt(hits.get(0).value) * Integer.parseInt(hits.get(1).value)
                            : 0;
                })
                .sum();
    };
    ...
    @Override
    public void run() {
        Solver.solve(InputFile.of(Main.class, "sample.txt"), solution1, Optional.of(4361));

        Solver.solve(InputFile.of(Main.class, "input.txt"), solution1, Optional.of(527364));

        Solver.solve(InputFile.of(Main.class, "sample.txt"), solution2, Optional.of(467835));

        Solver.solve(InputFile.of(Main.class, "input.txt"), solution2, Optional.of(79026871));
    }
