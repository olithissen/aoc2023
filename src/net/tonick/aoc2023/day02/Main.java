...
public class Main extends Solver {
    ...
    private static final Predicate<Game> validateGame = game -> game.draws.stream().allMatch(draw -> draw.red <= 12 && draw.green <= 13 && draw.blue <= 14);

    private static final Function<Game, Draw> createMetaDraw = game -> {
        var maxRed = game.draws.stream().mapToInt(d -> d.red).max().orElse(0);
        var maxGreen = game.draws.stream().mapToInt(d -> d.green).max().orElse(0);
        var maxBlue = game.draws.stream().mapToInt(d -> d.blue).max().orElse(0);
